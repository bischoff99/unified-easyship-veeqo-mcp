...
class MonitoringSystem extends EventEmitter {
  ...
  private readonly MEMORY_WARNING_THRESHOLD =
    Number(process.env.MEMORY_WARNING_THRESHOLD) || 400; // MB
  private readonly MEMORY_ERROR_THRESHOLD =
    Number(process.env.MEMORY_ERROR_THRESHOLD) || 500; // MB
  private readonly API_WARNING_THRESHOLD =
    Number(process.env.API_WARNING_THRESHOLD) || 2000; // ms
  private readonly API_ERROR_THRESHOLD =
    Number(process.env.API_ERROR_THRESHOLD) || 5000; // ms
  ...
  getSystemMetrics(): SystemMetrics {
    ...
    const failedApiCalls = recentApiCalls.filter(
      (m) => m.labels?.success === "false",
    );

    const totalRequests = recentApiCalls.reduce(
      (sum, metric) => sum + metric.value,
      0,
    );
    const errorRate =
      totalRequests > 0 ? (failedApiCalls.length / totalRequests) * 100 : 0;
    const averageResponseTime =
      recentApiMetrics.length > 0
        ? recentApiMetrics.reduce((sum, metric) => sum + metric.value, 0) /
...
export function withMonitoring<T extends any[], R>(
  ...
  return async (..._args: T): Promise<R> => {
    ...
    try {
      ...
      monitoring.recordMetric(`${name}_duration`, duration, {
        ...labels,
        success: "false",
      });
      ...
      if (error instanceof Error) {
        monitoring.recordError(error, {
          operation: name,
          args: _args.slice(0, 2),
        }); // Limit args to prevent huge logs
      }
