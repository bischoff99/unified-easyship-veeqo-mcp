...
export function addInventoryTools(server: FastMCP, veeqoClient: VeeqoClient) {
  ...
  server.addTool({
    ...
    parameters: z.object({
      page: z.number().min(1).default(1).describe('Page number for pagination'),
      per_page: z.number().min(1).max(100).default(25).describe('Number of customers per page'),
    }),
    execute: async (args) => {
      const startTime = Date.now();
      try {
  ...
  server.addTool({
    ...
    parameters: z.object({
      page: z.number().min(1).default(1).describe('Page number for pagination'),
      per_page: z.number().min(1).max(100).default(25).describe('Number of suppliers per page'),
    }),
    execute: async (args) => {
      const startTime = Date.now();
      try {
  ...
  server.addTool({
    ...
    parameters: z.object({
      ...
      carrier: z.string().optional().describe('Preferred carrier (DHL, FedEx, UPS)'),
    }),
    execute: async (args) => {
      const startTime = Date.now();
      try {
  ...
  server.addTool({
    ...
    parameters: z.object({
      order_id: z.number().describe('Veeqo order ID'),
      carrier: z.string().optional().describe('Shipping carrier (USPS, UPS, FedEx, DHL)'),
      service: z.string().optional().describe('Shipping service type'),
      ...
      })).optional().describe('Specific line items to ship (if not provided, ships all items)'),
    }),
    execute: async (args) => {
      const startTime = Date.now();
      try {
  ...
  server.addTool({
    ...
    parameters: z.object({
      ...
      status: z.enum(['pending', 'shipped', 'delivered', 'returned', 'cancelled']).optional().describe('Shipment status'),
      shipped_at: z.string().optional().describe('ISO date string when shipped'),
      delivered_at: z.string().optional().describe('ISO date string when delivered'),
    }),
    execute: async (args) => {
      const startTime = Date.now();
      try {
  ...
  server.addTool({
    ...
    parameters: z.object({
      ...
      carrier: z.string().optional().default('amazon_shipping_v2').describe('Carrier to get quotes from'),
    }),
    execute: async (args) => {
      const startTime = Date.now();
      try {
  ...
  server.addTool({
    ...
    parameters: z.object({
      ...
      days: z.number().min(1).max(365).default(30).describe('Number of days to forecast'),
    }),
    execute: async (args) => {
      const startTime = Date.now();
      try {
  ...
  server.addTool({
    ...
    parameters: z.object({
      ...
      is_default: z.boolean().optional().default(false).describe('Set as default location'),
    }),
    execute: async (args) => {
      const startTime = Date.now();
      try {
