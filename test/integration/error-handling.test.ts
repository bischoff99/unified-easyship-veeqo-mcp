...
describe('Error Handling Integration', () => {
  ...
  test('should determine default status codes correctly', () => {
    const badRequestError = createError(ErrorCode.INVALID_PARAMS, 'Bad request');
    const unauthorizedError = createError(ErrorCode.UNAUTHORIZED, 'Unauthorized');
    ...
    const internalError = createError(ErrorCode.INTERNAL_ERROR, 'Internal error');
  ...
  test('should calculate retry delays correctly', () => {
    const rateLimitError = createError(ErrorCode.RATE_LIMITED, 'Rate limited', { retryAfter: 30 });
    const serviceError = createError(ErrorCode.SERVICE_UNAVAILABLE, 'Service unavailable');
  ...
  test('should create validation errors', () => {
    const validationError = createValidationError('email', 'invalid', 'valid email');
  ...
  test('should convert errors to response format', () => {
    const mcpError = createError(ErrorCode.API_ERROR, 'API error', { service: 'test' });
    const regularError = new Error('Regular error');
