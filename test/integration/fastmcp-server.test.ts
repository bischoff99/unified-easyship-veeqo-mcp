...
const mockVeeqoClient = {
  getInventoryLevels: vi
    .fn()
    .mockResolvedValue(mockVeeqoApiResponses.getInventoryLevels()),
  updateInventoryLevels: vi
    .fn()
    .mockResolvedValue(mockVeeqoApiResponses.updateInventoryLevels()),
...
describe("FastMCP Server Integration", () => {
  ...
  describe("External Service Integration", () => {
    ...
    it("should return mocked inventory data", async () => {
      const inventory = await mockVeeqoClient.getInventoryLevels(
        ["123456"],
        ["339686"],
      );
  ...
  describe("Label Creation Service", () => {
    ...
    it("should handle tracking successfully", async () => {
      const tracking =
        await mockEasyPostClient.trackShipment("1Z999AA1234567890");
  ...
  describe("Inventory Management Integration", () => {
    it("should handle inventory levels query", async () => {
      const result = await mockVeeqoClient.getInventoryLevels(
        ["123456"],
        ["339686"],
      );
      ...
      expect(mockVeeqoClient.getInventoryLevels).toHaveBeenCalledWith(
        ["123456"],
        ["339686"],
      );
    });
    ...
    it("should handle inventory updates", async () => {
      ...
      expect(result).toBeDefined();
      expect(mockVeeqoClient.updateInventoryLevels).toHaveBeenCalledWith(
        updates,
      );
    });
  });
  ...
  describe("Order Processing Integration", () => {
    ...
    it("should handle order updates", async () => {
      ...
      expect(result).toBeDefined();
      expect(mockVeeqoClient.updateOrder).toHaveBeenCalledWith(
        "445566",
        orderData,
      );
    });
  ...
  describe("Error Handling", () => {
    it("should handle API errors in EasyPost client", async () => {
      ...
      mockEasyPostClient.getRates.mockRejectedValueOnce(
        new Error("EasyPost API Error"),
      );

      await expect(
        mockEasyPostClient.getRates(
          mockEasyPostAddress,
          mockEasyPostAddress,
          mockEasyPostParcel,
        ),
    ...
    it("should handle API errors in Veeqo client", async () => {
      // Mock a failing API call
      mockVeeqoClient.getInventoryLevels.mockRejectedValueOnce(
        new Error("Veeqo API Error"),
      );
  ...
  describe("Resource Data Sources", () => {
    it("should provide shipping rates via EasyPost integration", async () => {
      ...
      expect(mockEasyPostClient.getRatesByZip).toHaveBeenCalledWith(
        "94105",
        "10001",
      );
    });
    ...
    it("should provide product inventory data", async () => {
      ...
      expect(mockVeeqoClient.getProductInventory).toHaveBeenCalledWith(
        "123456",
      );
    });
  });
