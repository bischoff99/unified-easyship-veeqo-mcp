...
async function testVeeqoProduction() {
  ...
  if (!process.env.VEEQO_API_KEY || process.env.VEEQO_API_KEY === "mock") {
    ...
    console.log("   or");
    console.log(
      "   export VEEQO_API_KEY=your_key_here && node test-veeqo-production.js",
    );
    return;
  }

  console.log(
    `🔑 Using Veeqo API Key: ${process.env.VEEQO_API_KEY.substring(0, 8)}...`,
  );
  console.log(
  ...
  try {
    ...
    try {
      ...
      if (locations.length > 0) {
        ...
        locations.forEach((location, index) => {
          console.log(
            `      ${index + 1}. ${location.name} (ID: ${location.id})`,
          );
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (inventory.length > 0) {
        ...
        Object.entries(summary).forEach(([location, data]) => {
          console.log(
            `      ${location}: ${data.total} units across ${data.products} products`,
          );
        });
      }
    } catch (error) {
    ...
    try {
      ...
      if (orders.length > 0) {
        ...
        console.log(
          `   ✅ Success: Retrieved order #${orderDetails.order_number}`,
        );
  ...
  console.log(
    "   - This confirms your Veeqo integration is ready for production",
  );
}
