  ...
  try {
    for (let i = 0; i < testDestinations.length; i++) {
      ...
      try {
        ...
        const uspsRates = await client.getRates(
          fromAddress,
          destination,
          parcel,
          ["USPS"],
        ...
        console.log(
          `   ✅ USPS International: ${uspsRates.length} rates found`,
        );
        ...
        if (allRates.length === 0) {
          ...
          const lightRates = await client.getRates(
            fromAddress,
            destination,
            lightParcel,
          );
    ...
    console.log(
      "   4. Verify your account has international shipping permissions",
    );
  } catch (error) {
