...
async function testVeeqoDirect() {
  ...
  try {
    ...
    try {
      ...
      if (orders.length > 0) {
        console.log(`   📋 First order: #${orders[0].order_number} (ID: ${orders[0].id})`);
      }
    } catch (error) {
      console.log(`   ❌ Error: ${error.message}`);
    }
    ...
    try {
      ...
      if (suppliers.length > 0) {
        console.log(`   🏭 First supplier: ${suppliers[0].name} (ID: ${suppliers[0].id})`);
      }
    } catch (error) {
      console.log(`   ❌ Error: ${error.message}`);
    }
    ...
    try {
      ...
      if (warehouses.length > 0) {
        console.log(`   🏢 First warehouse: ${warehouses[0].name} (ID: ${warehouses[0].id})`);
      }
    } catch (error) {
      console.log(`   ❌ Error: ${error.message}`);
    }
