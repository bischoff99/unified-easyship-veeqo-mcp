  ...
  for (const endpoint of endpointPatterns) {
    try {
      ...
      if (analysis.sampleData) {
        console.log(
          `   🔍 Sample: ${JSON.stringify(analysis.sampleData).substring(0, 100)}...`,
        );
      }
      console.log("");
...
function analyzeResponse(endpoint, response) {
  ...
  if (Array.isArray(response)) {
    ...
    if (response.products) {
      ...
      fields =
        response.products.length > 0 ? Object.keys(response.products[0]) : [];
      sampleData = response.products.length > 0 ? response.products[0] : null;
    } else if (response.orders) {
      type = "orders_wrapper";
      fields =
        response.orders.length > 0 ? Object.keys(response.orders[0]) : [];
      sampleData = response.orders.length > 0 ? response.orders[0] : null;
    } else if (response.locations) {
      type = "locations_wrapper";
      fields =
        response.locations.length > 0 ? Object.keys(response.locations[0]) : [];
...
function generateAPIDocumentation(endpoints) {
  ...
  successful.forEach(([endpoint, analysis]) => {
    ...
    if (analysis.sampleData) {
      console.log(
        `  Sample: ${JSON.stringify(analysis.sampleData, null, 2).substring(0, 200)}...`,
      );
    }
  });
  ...
  console.log(
    `Success rate: ${((successful.length / endpoints.size) * 100).toFixed(1)}%`,
  );
}
