...
async function testCustomsInfo() {
  console.log(
    "🌍 Testing International Shipping with Customs Information...\n",
  );
  ...
  try {
    ...
    if (response.rates && response.rates.length > 0) {
      ...
      console.log(
        `   🏷️  Attempting to buy ${firstRate.carrier} ${firstRate.service}...`,
      );

      const buyResponse = await client.makeRequest(
        "POST",
        `/shipments/${response.id}/buy`,
        {
