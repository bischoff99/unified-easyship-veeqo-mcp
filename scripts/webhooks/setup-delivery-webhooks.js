...
async function setupDeliveryWebhooks() {
  ...
  if (
    ...
    console.log(
      "💡 Run: EASYPOST_API_KEY=your_key node setup-delivery-webhooks.js",
    );
    return;
  }
  ...
  const webhookUrl =
    process.env.WEBHOOK_URL || "https://your-server.com/webhooks/delivery";
  ...
  try {
    ...
    for (const event of easyPostEvents) {
      try {
        ...
        const response = await easyPostClient.makeRequest(
          "POST",
          "/webhooks",
          webhookData,
        );
    ...
    try {
      ...
      console.log(
        "   💡 Veeqo may not support webhooks, or endpoint is different",
      );
    }
