        ...
        return {
          CallExpression(node) {
            if (
              ...
              if (executeProperty?.value?.type === 'ArrowFunctionExpression' ||
                ...
                if (functionBody?.type === 'BlockStatement') {
                  ...
                  if (!hasTryCatch) {
                    context.report({
                      ...
                      message: 'FastMCP tool execute function should include proper error handling with try-catch blocks',
                    });
                  }
                }
              }
                ...
                if (params?.length >= 2) {
                  ...
                  if (contextParam?.type === 'ObjectPattern') {
                    ...
                    if (!hasStreamContent && !hasReportProgress) {
                      context.report({
                        node: executeProperty.value,
                        message: 'Consider using streamContent and reportProgress for better user experience in long-running operations',
                      });
                    }
                  }
                }
